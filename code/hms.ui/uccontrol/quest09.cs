...
namespace Hms.Ui
{
    public partial class Quest09 : UserControl, IQuest
    {
        public Quest09()
        {
            InitializeComponent();
        ...
        public List<EntityDicQnDetail> GetQnCtrls()
        {
            ...
            foreach (Control ctr in plBack.Controls)
            {
                if (ctr is DevExpress.XtraEditors.CheckEdit)
                {
                    ...
                    {
                        data.Add(new EntityDicQnDetail() { fieldId = ((DevExpress.XtraEditors.CheckEdit)ctr).Properties.AccessibleName, questName = "quest09" });
                    }
                }
                if (ctr is DevExpress.XtraEditors.LabelControl)
                {
                    data.Add(new EntityDicQnDetail() { fieldId = ctr.Name, questName = "quest09" });
                }
                ...
                if (ctrl is DevExpress.XtraEditors.LabelControl)
                {
                    if (ctrl.Name.Contains("F"))
                    {
                        ...
                        xml.Append("<ctrl ctrlname=\"" + ctrl.Name +
                                   "\" ctrlText=\"" + ctrl.Text.Replace("<", "&lt;").Replace(">", "&gt;").Replace("'", "&aops;").Replace("\"", "&quot;") +
